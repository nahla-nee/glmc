...
//vec2 function
inline bool vec2_equal(__m128 a, __m128 b){
	a = _mm_sub_ps(a, b);
...
inline bool vec2_equal_eps(__m128 a, __m128 b, float epsilon){
	a = _mm_sub_ps(a, b);
	a = _mm_and_ps(a, _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff)));
...
inline float vec2_len(__m128 a){
	vec2 tmp = GLMC_CAST(vec2, _mm_mul_ps(a, a));
	return sqrtf(tmp.x+tmp.y);
}
inline __m128 vec2_norm(__m128 a){
	vec2 tmp = GLMC_CAST(vec2, _mm_mul_ps(a, a));
	return _mm_div_ps(a, _mm_set1_ps(sqrtf(tmp.x+tmp.y)));
}
inline float vec2_dot(__m128 a, __m128 b){
	vec2 tmp = GLMC_CAST(vec2, _mm_mul_ps(a, b));
	return (tmp.x + tmp.y);
}
inline __m128 vec2_set(float x, float y){
	return _mm_set_ps(0.f, 0.f, y, x);
}
inline __m128 vec2_mulMat(__m128 a, mat2 b){
...
//vec3 functions
inline bool vec3_equal(__m128 a, __m128 b){
	a = _mm_sub_ps(a, b);
	a = _mm_and_ps(a, _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff)));
...
inline bool vec3_equal_eps(__m128 a, __m128 b, float epsilon){
	a = _mm_sub_ps(a, b);
	a = _mm_and_ps(a, _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff)));
...
//vec4 functions
inline bool vec4_equal(__m128 a, __m128 b){
	a = _mm_sub_ps(a, b);
	a = _mm_and_ps(a, _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff)));
...
inline bool vec4_equal_eps(__m128 a, __m128 b, float epsilon){
	a = _mm_sub_ps(a, b);
	a = _mm_and_ps(a, _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff)));
...
inline float vec4_len(__m128 a){
	vec4 tmp = GLMC_CAST(vec4, _mm_mul_ps(a, a));
	return sqrtf(tmp.x+tmp.y+tmp.z+tmp.w);
}
inline __m128 vec4_norm(__m128 a){
	vec4 tmp = GLMC_CAST(vec4, _mm_mul_ps(a, a));
	return _mm_div_ps(a, _mm_set1_ps(sqrtf(tmp.x+tmp.y+tmp.z+tmp.w)));
}
inline float vec4_dot(__m128 a, __m128 b){
	vec4 tmp = GLMC_CAST(vec4, _mm_mul_ps(a, b));
	return (tmp.x+tmp.y+tmp.z+tmp.w);
}
inline __m128 vec4_set(float x, float y, float z, float w){
	return _mm_set_ps(w, z, y, x);
}
inline __m128 vec4_mulMat(__m128 a, mat4 b){
	vec4 tmp = GLMC_CAST(vec4, a);
	__m128 res = _mm_mul_ps(b.mat[0], _mm_set1_ps(tmp.x));
	res = _mm_add_ps(res, _mm_mul_ps(b.mat[1], _mm_set1_ps(tmp.y)));
	res = _mm_add_ps(res, _mm_mul_ps(b.mat[2], _mm_set1_ps(tmp.z)));
...
inline __m128 vec_set1(float x){
	return _mm_set1_ps(x);
}
inline __m128 vec_add(__m128 a, __m128 b){
	return _mm_add_ps(a, b);
}
inline __m128 vec_sub(__m128 a, __m128 b){
	return _mm_sub_ps(a, b);
}
inline __m128 vec_mul(__m128 a, __m128 b){
	return _mm_mul_ps(a, b);
}
inline __m128 vec_div(__m128 a, __m128 b){
	return _mm_div_ps(a, b);
}
inline __m128 vec_addScal(__m128 a, float scalar){
	return _mm_add_ps(a, _mm_set1_ps(scalar));
}
inline __m128 vec_subScal(__m128 a, float scalar){
	return _mm_sub_ps(a, _mm_set1_ps(scalar));
}
inline __m128 vec_mulScal(__m128 a, float scalar){
	return _mm_mul_ps(a, _mm_set1_ps(scalar));
}
inline __m128 vec_divScal(__m128 a, float scalar){
...
inline __m128 vec_ceil(__m128 a){
	return _mm_ceil_ps(a);
}
inline __m128 vec_floor(__m128 a){
	return _mm_floor_ps(a);
}
inline __m128 vec_round(__m128 a){
